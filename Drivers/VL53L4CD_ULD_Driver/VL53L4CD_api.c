...
VL53L4CD_Error VL53L4CD_GetSWVersion(VL53L4CD_Version_t *p_Version) {
...
VL53L4CD_Error VL53L4CD_SensorInit(Dev_t dev) {
    ...
    for (Addr = (uint8_t)0x2D; Addr <= (uint8_t)0x87; Addr++) {
        status |= VL53L4CD_WrByte(
            dev, Addr, VL53L4CD_DEFAULT_CONFIGURATION[Addr - (uint8_t)0x2D]);
    }
    ...
    status |= VL53L4CD_WrByte(
        dev, VL53L4CD_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, (uint8_t)0x09);
...
VL53L4CD_Error VL53L4CD_SetRangeTiming(Dev_t dev, uint32_t timing_budget_ms,
    ...
    /* Sensor runs in autonomous low power mode */
        ...
        status |= VL53L4CD_RdWord(dev, VL53L4CD_RESULT__OSC_CALIBRATE_VAL,
                                  &clock_pll);
...
VL53L4CD_Error VL53L4CD_GetRangeTiming(Dev_t dev, uint32_t *p_timing_budget_ms,
    ...
    status |= VL53L4CD_RdDWord(dev, VL53L4CD_INTERMEASUREMENT_MS, &tmp);
    status |=
        VL53L4CD_RdWord(dev, VL53L4CD_RESULT__OSC_CALIBRATE_VAL, &clock_pll);
    clock_pll = clock_pll & (uint16_t)0x3FF;
...
VL53L4CD_Error VL53L4CD_SetXtalk(Dev_t dev, uint16_t XtalkValueKcps) {
    ...
    status |=
        VL53L4CD_WrWord(dev, VL53L4CD_XTALK_X_PLANE_GRADIENT_KCPS, 0x0000);
    status |=
        VL53L4CD_WrWord(dev, VL53L4CD_XTALK_Y_PLANE_GRADIENT_KCPS, 0x0000);
...
VL53L4CD_Error VL53L4CD_GetXtalk(Dev_t dev, uint16_t *p_xtalk_kcps) {
    ...
    status |=
        VL53L4CD_RdWord(dev, VL53L4CD_XTALK_PLANE_OFFSET_KCPS, p_xtalk_kcps);
...
VL53L4CD_Error VL53L4CD_SetSignalThreshold(Dev_t dev, uint16_t signal_kcps) {
    ...
    status |= VL53L4CD_WrWord(dev, VL53L4CD_MIN_COUNT_RATE_RTN_LIMIT_MCPS,
                              signal_kcps >> 3);
    return status;
}
...
VL53L4CD_Error VL53L4CD_GetSignalThreshold(Dev_t dev, uint16_t *p_signal_kcps) {
    ...
    status |=
        VL53L4CD_RdWord(dev, VL53L4CD_MIN_COUNT_RATE_RTN_LIMIT_MCPS, &tmp);
    *p_signal_kcps = tmp << 3;
...
VL53L4CD_Error VL53L4CD_SetSigmaThreshold(Dev_t dev, uint16_t sigma_mm) {
    ...
    if (sigma_mm > (uint16_t)((uint16_t)0xFFFF >> 2)) {
        ...
        status |= VL53L4CD_WrWord(dev, VL53L4CD_RANGE_CONFIG__SIGMA_THRESH,
                                  sigma_mm << 2);
    }
...
VL53L4CD_Error VL53L4CD_GetSigmaThreshold(Dev_t dev, uint16_t *p_sigma_mm) {
    ...
    status +=
        VL53L4CD_RdWord(dev, VL53L4CD_RANGE_CONFIG__SIGMA_THRESH, p_sigma_mm);
...
VL53L4CD_Error VL53L4CD_StartTemperatureUpdate(Dev_t dev) {
    ...
    status |= VL53L4CD_WrByte(
        dev, VL53L4CD_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, (uint8_t)0x81);
    ...
    status += VL53L4CD_WrByte(
        dev, VL53L4CD_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, 0x09);
